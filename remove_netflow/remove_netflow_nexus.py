...
def main():
    ...
    def trunk_interfaces():
        ...
        for line in lines:
            ...
            config.append(" no ip flow monitor MONITOR_NAME sampler SAMPLER_NAME input")
            config.append("!")
        "\n".join(config)
    ...
    def routed_interfaces():
        config = []
        interface_list = connect.send_command("show int status | in routed")
        lines = interface_list.split()
        for line in lines:
            if "Po" in line:
                continue
            interface = line.split()[0]
            config.append("interface " + interface)
            config.append(" no ip flow monitor MONITOR_NAME input sampler SAMPLER_NAME")
            config.append("!")
        "\n".join(config)
        connect.send_config_set(config)
